...
final class NamePrettifier
{
    ...
    public function prettifyTestClass(string $className): string
    {
        ...
        if (substr($className, -1 * strlen('CartTest')) === 'CartTest') {
            $className = substr($className, 0, strlen($className) - strlen('CartTest'));
        }

        if (strpos($className, 'Tests') === 0) {
            $className = substr($className, strlen('Tests'));
        } elseif (strpos($className, 'CartTest') === 0) {
            $className = substr($className, strlen('CartTest'));
        }
